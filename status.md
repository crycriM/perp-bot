...
| Deploy-time topology | **Helper tested; routing now proven through the HB connector itself.** The quote gate showed a `PerpMMController` on `e2_mm1` and one on `e2_mm2` each landing orders **only** on their own subaccount (raw-SDK cross-check of all three accounts). | Add the HB script-strategy/config-loader call to `validate_controller_topology()` before controllers start, and run the two legs **concurrently** — both gate runs were sequential, and the three accounts still share one agent signer key. |
