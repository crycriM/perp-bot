...
| Basket rebalancer | **Live shadow ran 2026-09-15** (real HL positions/mids, e2_mm1/e2_mm2; `scripts/run_rebalancer_shadow.py`, logs `perp-bot/logs/rebalancer_shadow_*.jsonl`). Capacity guard added the same day: flat 300 USDC accounts would need ~$1363 vs a ~$749 budget (§1.4, 2.5x equity), so corrections are skipped and logged. | Demonstrate a bounded correction through the selected execution body (needs OPMS running + plan-funded accounts). |
