...
- ⚠ **The pinned Hummingbot checkout carries one local (uncommitted) patch:**
  ...
  must be `< 0.01`).
- Unit suites green on 2026-09-16: **mm-core 121**, **perp-bot 74**,
  **hb-enhanced-opms 155** stub tests, plus **35 real-Hummingbot tests**
...
| Backtest and basket preparation | **Implemented / calibrated; rebalancer feeds now live** | BTC gates passed; the four-leg ETH/SOL basket and its rebalancer were added and calibrated on 2026-08-31. 2026-09-15: the rebalancer runs in shadow against real HL positions/mids (`scripts/run_rebalancer_shadow.py`, `rebalancer_feeds.py`), which exposed a sizing gap now closed by the leverage-aware `max_gross_notional_multiple` guard. The basket now uses 6x leverage so its current ~$1,350 target gross fits the ~$300 accounts within the buffered initial-margin budget. | The newer ETH/SOL replay is candle-derived, not tick replay; the rebalancer's OPMS intent path (execution) is still untested. |
| `PerpMMController` code path | **Runs against real Hummingbot; soak remediation implemented offline, live rerun pending** | The controller reuses `Keeper` via `InProcessClient`, maps quote/execution intents to HB executor actions, wires a `FillObserver`, and routes every intent to an executor config the real `ExecutorOrchestrator` can instantiate (39 real-HB tests). The 2026-09-16 remediation adds bounded quote-stop flattening, venue-order-id liveness with cooldown, two-phase quote refresh, target/flat fill bounds, request deadlines with 429-aware circuits, and an MQTT-isolated launcher. Earlier quote, fill, de-risk, and emergency paths passed live; these new protections have not yet been rerun live. | Rerun the single-account safety soak, then parity vs the native oracle. |
| Hummingbot ↔ HL connection | **Read-only smoke + live quote/cancel gate passed on mainnet** | The real `HyperliquidPerpetualDerivative` builds with `use_vault=True` for `e2_mm1` (reads its 300 USD unified balance) and `use_vault=False` for `e2_main` (131.98 USD); the real `PerpMMController.on_start()` registers the `FillObserver` on that connector, `update_processed_data()` reads a live mid + funding (`1.25e-05`, after the connector patch) and returns positive equity. A mainnet credential importer for master/subaccount shapes exists and dry-runs correctly. `scripts/run_hb_mainnet_quote_gate.py` then drove the same controller through real `OrderExecutor`s to **resting mainnet orders** on `e2_mm1` and `e2_mm2` (see the 2026-09-15 achievement row). `scripts/run_hb_mainnet_derisk_gate.py` then produced real fills, reconciled the position, and flattened it through the de-risk and emergency executors. The basket configs now request 6x per pair; HB's startup applies that leverage to the ETH and SOL markets. | Import into HB's encrypted store (the gates inject credentials directly) and run under a real HB strategy/Clock; parity. |
...
| 2026-09-15 | **Real Hummingbot launch path built (step A of the single-account plan).** | `hb-enhanced-opms/deploy/`: HB V2 script `opms_perp_mm.py` (refuses to start on topology violations or credentials routed to another account — this is the first call site of `validate_controller_topology()`), controller shim, installer, and the initial `e2_mm1` shadow config. Verified offline with HB's own loader logic: script/config classes resolve, `PerpMMController` loads from the YAML, markets derive to `hyperliquid_perpetual: ETH-USD`, and the routing guard accepts `e2_mm1`'s address and rejects master credentials. Fixed on the way: HB's `add_controller()` never passes `update_interval`, so controllers would have cycled at 1 s (now a config field, default 5 s); `opms/connectors/topology.py` kept its own capability table that still called Lighter hedge-mode (now delegates to `perp_bot.venue_capabilities`). Added controller `shadow_mode`. The dual basket expansion and live launch evidence are recorded in the 2026-09-16 achievement below. |
...
| 2026-09-15 | **De-risk gate rerun passed** after the PA `COMPLETED` fix and a gate sampling fix; **rebalancer live shadow + capacity guard**. | `run_hb_mainnet_derisk_gate.py` rerun (`logs/derisk_gate_20260915T171712`): its first attempt failed only because the ~9.4 s pump cadence missed a reduce-only order that rested <8 s — a fast venue-order poller (`_sleep_with_order_poll`) now records those, and the rerun passed: reduce-only oid observed resting, PA `COMPLETED` with 0.012/0.012 executed, de-risk flat in 20.7 s, emergency flat in 15.0 s, teardown 0 orders / 0 position. Separately, `perp-bot/scripts/run_rebalancer_shadow.py` + `rebalancer_feeds.py` (7 tests) ran the four-leg basket rebalancer in shadow against real HL positions/mids, exposing an underfunded-account over-correction (~$1363 vs the §1.4 ~$749 budget on 300 USDC accounts); closed with a `max_gross_notional_multiple` guard (2 tests, skip-and-log). perp-bot suite: 61 passed.
| 2026-09-16 | **Concurrent read-only HL follow-up passed.** | Real-HB connector/controller smokes for `e2_mm1` and `e2_mm2` ran concurrently with no orders and read the correct vault routing, positive equity, and plausible funding (`8.2838e-06`). A fresh live-feed rebalancer shadow observed flat portfolio net exposure and emitted four capacity-guard skips under the old 1x basket configuration; no correction was sent because each account still has only 300 USDC. Configured-environment tests also passed: mm-core 121, rebalancer/feed/OPMS 17 (1 skipped), real-HB 34 (2 mainnet-gated skips). Concurrent writes remain blocked by the shared signer and HB's single connector credential slot.
| 2026-09-16 | **Dual real-Hummingbot basket shadow startup passed.** | `hb-enhanced-opms/deploy/run_dual_shadow_session.sh 30` created two disposable Hummingbot runtimes and encrypted stores, imported `e2_mm1` and `e2_mm2` separately, and started both `opms_perp_mm` processes concurrently. Each loaded its two tilted ETH/SOL controllers, initialized both live order books, and emitted four fresh decision logs with no order executor events. The YAML loader/routing validator passed for both configs. The two vault addresses are distinct; the agent signer remains shared.
| 2026-09-16 | **Collateral adaptation applied.** | The live HL mids make the configured `0.4 ETH / 4 SOL` target about $1,350 gross per subaccount. All four basket controllers now use 6x leverage, giving about $225 initial margin against the current ~$300 collateral. `BasketRebalancer` now evaluates target initial margin using each pair's leverage and retains the buffered allowance; its tests prove the target passes at 6x and is blocked at the old reference sizing. A fresh live read-only cycle (`run_rebalancer_shadow.py --cycles 1`) emitted four `subaccount_imbalance` records instead of four capacity skips; no intents were sent. |
| 2026-09-16 | **Single-account 30-minute runtime soak completed; behavioral safety gate failed.** | `hb-enhanced-opms/deploy/run_single_live_soak.sh 1800` ran both calibrated e2_mm1 ETH/SOL controllers at 6x for 1,800 seconds. It produced 346 ETH and 345 SOL live decision records, 220 maker order creations, and four unique maker fills. The monitor saw maximum gross exposure $379.29, maximum initial margin $63.22, minimum margin health 98.83%, and six scoped orders at most. The controller kept ticking, but regime stops left -2 SOL and -0.0231 ETH unquoted for long intervals; the worst mark was about -$2.65 before fees. One post-only price race was rejected and retried. Teardown market-closed 0.0231 ETH and 2 SOL, then verified 729.333126 USDC, zero ETH/SOL positions, and zero ETH/SOL orders. No confirmed HL 429/rate-limit or HTTP timeout was logged, but the optional MQTT bridge repeatedly refused connections and timed out. Artifacts: `hb-enhanced-opms/logs/live_soak_20260916T132141/`. |
| 2026-09-16 | **Post-soak safety remediation implemented and verified offline.** | `STOP_QUOTING` now cancels and flattens residual inventory through one bounded reduce-only PA child (15-second passive deadline); quote sizes cannot cross structural `q*` on a full fill or flip through flat; both HB and native OPMS propagate venue reduce-only. HB now monitors actual quote order ids, opens a 30-second recovery circuit after 15 seconds missing, and refreshes cancel-then-create. Controller/native HL calls have 5-second deadlines plus exponential timeout/429 backoff; the bounded HB launchers bypass MQTT. Verification: perp-bot 81 passed/1 skipped; HB stub 165 passed; real HB 39 passed/2 skipped; focused native OPMS intent/algorithm/adapter/resilience safety tests pass (the repository-wide native suite still has unrelated pre-existing collection/environment failures). Live-soak rerun remains required. |
...
| Basket rebalancer | **Live shadow passed 2026-09-15 and 2026-09-16** (real HL positions/mids, e2_mm1/e2_mm2; `scripts/run_rebalancer_shadow.py`, latest artifact `logs/rebalancer_shadow_20260916T112400_followup.jsonl`). Capacity guard correctly skips the ~$1350 tilt on flat 300-USDC accounts. | Demonstrate a bounded correction through the selected execution body (needs OPMS running + plan-funded accounts). |
...
| Phase 2 executor routing | **Implemented; verified against real HB and live on mainnet, except the new quote-stop route.** | `tests_real/test_perp_mm_controller_real_hb.py` instantiates the real controller path and asserts every urgency (quote / passive / normal / immediate / emergency) produces a config whose `.type` is in HB's real `ExecutorOrchestrator._executor_mapping`. Quote (`run_hb_mainnet_quote_gate.py`), de-risk and emergency (`run_hb_mainnet_derisk_gate.py`) routes are proven live to fills. `immediate` now maps to one full-size reduce-only PA child and crosses after 15 seconds; unsafe opening-capable TWAP is no longer used for quote stops. | Exercise the new immediate quote-stop flatten in the safety-soak rerun. |
...
| Deploy-time topology | **Dual real-HB shadow startup/config passed 2026-09-16** (`deploy/hummingbot/scripts/opms_perp_mm.py`, `validate_hb_deploy_configs.py`, `run_dual_shadow_session.sh`). Both instances loaded two controllers, used separate credential stores, started concurrently, and initialized ETH/SOL without order executor events; the two vault addresses are distinct. The quote gate also showed each account's orders landing **only** on its own subaccount (raw-SDK cross-check of all three accounts). | Run the two instances with writes **concurrently** — the accounts still share one agent signer key, and persistent HB credential stores need importing. |
...
| Phase 2 live gate | **Passed on mainnet 2026-09-15** (no HL testnet credentials exist in `.env`, so both gates ran on mainnet at micro size, matching the 2026-09-14 raw-SDK precedent). Quote create → rest → refresh → cancel → clean stop on `e2_mm1` and `e2_mm2`; real fill, position reconciliation, passive de-risk to `COMPLETED`, emergency exit to flat, FillObserver analytics, and clean stop on `e2_mm1` (`logs/derisk_gate_20260915T122339`). The previously defined executor gate criteria were met; the later single-account soak exposed a separate quote-liveness/position-safety gap. The emergency market-order latency first measured 12.2 s against the 10 s criterion (10 s child limit + executor tick + cancel acknowledgement); the emergency `child_order_time_limit` was set to **5 s** by decision on 2026-09-15 and re-measured at **8.7 s** from the controller's decision (flat in 21.7 s, `logs/derisk_gate_20260915T144500`). The gate now fails if it exceeds 10 s. **Rerun 2026-09-15 (`logs/derisk_gate_20260915T171712`) after the PA `COMPLETED` fix and a gate instrumentation fix** (a fast venue-order poller: the ~9.4 s pump cadence had missed an order that rested <8 s): **PASSED** — reduce-only order observed resting, PA `COMPLETED` with 0.012/0.012 executed, de-risk flat in 20.7 s, emergency flat in 15.0 s, teardown 0 orders / 0 position. |
| Single-account live soak | **Prior runtime passed but safety gate failed; remediation is implemented, rerun pending.** | The 1,800-second e2_mm1 run exposed unquoted residual inventory and MQTT retry noise. Quote-stop flattening, quote liveness/cooldown, two-phase refresh, bounded fill sizes, MQTT isolation, and timeout/429 circuits now have offline coverage. No live run has exercised the changed behavior yet. | Rerun the bounded e2_mm1 soak, inject or observe a controlled venue fault, and require zero unbounded residual-inventory intervals plus clean teardown. |
...
5. **Test deployment topology and the basket.** Invoke topology validation in
   ...
   **Dual read-only startup/config done 2026-09-16:** both real-HB instances
   loaded their two tilted controllers concurrently through separate encrypted
   stores, initialized ETH/SOL, and emitted no order executor events. The
   rebalancer feed shadow is also complete; writes still require separate agent
   signer keys and plan-funded accounts.
...
- ~~**Drawdown stop is blind to open-position losses on HL unified
  ...
  `crossMaintenanceMarginUsed` (exact to 1e-4 in the same samples) —
  **wired into a margin-health stop 2026-09-15:** `RiskConfig` gained
  `margin_health_soft=0.20` / `margin_health_hard=0.10` (ratios of
  available-after-maintenance to equity), `RiskPolicy.evaluate` takes
...
- ~~**A PA whose child is skipped still reports `COMPLETED`.**~~ **Fixed
  2026-09-15** (`clmm-animation/docs/common-mm-remaining-tasks-BCDE.md`
  Stream D): skipped children get their own status, both executors close via
  `_final_close_type()` (COMPLETED only when executed >= total, else
  TIME_LIMIT/FAILED), and the de-risk gate now checks the executed size, not
  the close type. Live-validated in the `derisk_gate_20260915T171712` rerun.
